...
class FaceObject:
    ...
    def polygonOrient(self, corners):
        """
        Sets target positions along the perimeter of a transformed polygon.
        """
        if self.vert_count <= 0:
            return
        ...
        side_lengths = [0] * side_count
        perimeter = 0

        for i in range(side_count):
            x1, y1 = corners[i]
            x2, y2 = corners[(i + 1) % side_count]
            length = math.hypot(x2 - x1, y2 - y1)
            side_lengths[i] = length
            perimeter += length

        if perimeter <= 0:
            for vert in self.verts:
                vert.target_position = [center_x, center_y]
            return

        distance_step = perimeter / self.vert_count
        side_index = 0
        side_start_distance = 0
        side_end_distance = side_lengths[0]

        for i, vert in enumerate(self.verts):
            distance = i * distance_step
            while side_index < side_count - 1 and distance >= side_end_distance:
                side_start_distance = side_end_distance
                side_index += 1
                side_end_distance += side_lengths[side_index]

            side_length = side_lengths[side_index]
            t = 0 if side_length == 0 else (distance - side_start_distance) / side_length
            x1, y1 = corners[side_index]
            x2, y2 = corners[(side_index + 1) % side_count]
            x = x1 + (x2 - x1) * t
            y = y1 + (y2 - y1) * t
    ...
    def rectangleOrient(self):
        ...
        """
        half_width = self.transform.scale
        half_height = self.transform.scale * 0.5
        self.polygonOrient([
    ...
    def apply_shape_state(self, shape_state):
        ...
        elif shape_state == 'Rectangle':
            self.rectangleOrient()
        elif shape_state == 'Triangle':
            self.triangleOrient()
